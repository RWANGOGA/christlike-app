    ...
    <div className="flex bg-[#FBF7EE] min-h-screen">
      ...
      <div className="flex-1 ml-0 lg:ml-64 p-4 md:p-6 lg:p-8 max-w-6xl mx-auto w-full">
        <header className="mb-6 md:mb-8">
          <h1 className="font-[family-name:var(--font-display)] text-2xl md:text-3xl text-[#23213A]">Sermon Series</h1>
          <p className="text-[#23213A]/50 mt-1 text-sm md:text-base">Listen and grow through powerful messages.</p>
        </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {series.map((s) => (
              <Link 
                href={`/sermons/${s.id}`} 
                key={s.id} 
                className="bg-white rounded-2xl border border-[#23213A]/10 overflow-hidden hover:shadow-lg transition group cursor-pointer block"
              >
                <div className="h-40 md:h-48 bg-[#23213A] relative flex items-center justify-center">
                  ...
                  <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition flex items-center justify-center">
                    <div className="w-14 h-14 rounded-full bg-[#E3A857] flex items-center justify-center opacity-0 group-hover:opacity-100 transition transform scale-75 group-hover:scale-100">
                      <svg className="w-6 h-6 text-[#171B33] ml-1" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div className="p-4 md:p-5">
                  <h3 className="font-[family-name:var(--font-display)] text-base md:text-lg text-[#23213A] mb-1 line-clamp-2">{s.title}</h3>
                  <p className="text-sm text-[#23213A]/50 mb-3">{s.speaker_name}</p>
                  <p className="text-sm text-[#23213A]/60 line-clamp-3 leading-relaxed">{s.description}</p>
                </div>
              </Link>
            ))}
          </div>
