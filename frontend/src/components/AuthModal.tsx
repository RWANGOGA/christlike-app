...
export default function AuthModal({ initialMode, onSuccess, onClose }: AuthModalProps) {
  // 'mode' now handles 'login', 'register', and 'forgot'
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      // FastAPI expects 'email' as a query parameter for this endpoint
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-md p-8 relative shadow-2xl">
        ...
        <h2 className="font-[family-name:var(--font-display)] text-2xl text-[#5A4A4A] mb-2 text-center">
          {mode === 'login' && 'Welcome Back'}
          {mode === 'register' && 'Join Christ-Like'}
          {mode === 'forgot' && 'Reset Password'}
        </h2>
        
        {mode === 'forgot' && (
          <p className="text-sm text-[#5A4A4A]/60 text-center mb-6">
            Enter your email and we'll send you a link to reset your password.
          </p>
        )}

        {error && (
          <p className="text-red-600 text-sm mb-4 bg-red-50 p-3 rounded-lg text-center">{error}</p>
        )}
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-lg border border-[#5A4A4A]/15 text-sm focus:outline-none focus:border-[#D4A5A5] focus:ring-1 focus:ring-[#D4A5A5]"
            ...
            <button
              ...
              className="w-full text-sm text-[#5A4A4A]/60 hover:text-[#5A4A4A] transition text-center"
            >
              Back to Login
            </button>
          </form>
              ...
              {mode === 'login' && (
                <div className="flex justify-end mt-2">
                  <button
                    ...
                    className="text-xs text-[#D4A5A5] hover:text-[#C99595] font-medium transition"
                  >
        ...
        {/* Toggle between Login and Register (Hidden when in 'forgot' mode) */}
        {mode !== 'forgot' && (
